...
struct BumbleHomeView: View {
    ...
    @State private var currSwipeOffset: CGFloat = 0
...
//                BumbleCardView()
                ...
                .frame(maxHeight: .infinity) // This is make sure when loading, the header will be the top even when user is not loaded yet.
                .padding(4)
...
//                                        scaleMultiplier: ,
                onChanged: { dragOffset in
                    currSwipeOffset = dragOffset.width
                },
    ...
    private var overlaySwipeIndicators: some View {
        ZStack {
            Circle()
                ...
                .scaleEffect(abs(currSwipeOffset) > 100 ? 1.5 : 1.0)
                .offset(x: min(-currSwipeOffset, 150))
                .offset(x: -100)
                .frame(maxWidth: .infinity, alignment: .leading)
            ...
            Circle()
                ...
                .frame(width: 60, height: 60)
                .scaleEffect(abs(currSwipeOffset) > 100 ? 1.5 : 1.0)
                .offset(x: max(-currSwipeOffset, -150))
                .offset(x: 100)
                .frame(maxWidth: .infinity, alignment: .trailing)
        }
        .animation(.smooth, value: currSwipeOffset)
    }
}
